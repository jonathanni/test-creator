...
public class Import {
	...
	Test importNew(File file) { 
		...
		while(questionNumber >= 0) { 
			int s = line.indexOf(String.valueOf(questionNumber) + ".", latest); //gets number of question
			if (s == -1) { //if no questions left, exit loop
				break;
			}
			receiveQuestion(s);
			questionNumber++;
    ...
    /**
     * 
     * After a question has been found, this helper method will determine
     * what type of question it is and call the corresponding method
     * depending on this type.
     * 
     * @param start
     *            the starting index for the question; this is also the
     *            number of the question (ex. "1. ")
     */
	void receiveQuestion(int start) {
		...
		if (parenIndex == -1 || parenIndex > nextNumIndex) {
			...
			if (choicestr.equalsIgnoreCase("true") || choicestr.equalsIgnoreCase("false")){
    ...
    /**
     * 
     * Creates a new TFQuestion and adds it to the test;
     * also determines whether the answer is true or false.
     * 
     * @param q
    ...
    /**
     * 
     * Creates a new MCQuestion and adds it to the test;
     * also finds all answers and determines which is correct.
     * 
     * @param q
     *            the string that stores the question
     * @param p
	...
	void addMC(String q, int p) {
		...
		int endIndex;
		int endAllIndex = line.indexOf(String.valueOf(questionNumber + 1) + ".");
		int findID = 0;
		int correctID = 0;
		//gets answer choices
    ...
    /**
     * 
     * Creates a new FIBQuestion and adds it to the test;
     * also finds all blanks and corresponding correct answers.
     * 
     * @param q
     *            the string that stores the question
     * @param p
     *            the integer that stores the index of the first 'Blank';
     *            this gives a starting point for finding all blanks
     *            and answer choices
     */
	void addFIB(String q, int p) {
		...
		//finds blanks
		while(p != -1 && p < endAllIndex){	
			while(startIndex != -1 && startIndex < newlineIndex) { //finds answers
				endIndex = line.indexOf("\n", startIndex+1);
				if (endIndex == -1 || endIndex >= newlineIndex) {
					...
					question.addAnswer(blankIndex, line.substring(startIndex+5, endIndex));
					break;
				}
				question.setBlankSpaces(blankIndex+1);
				for (int i = 0; i < blankIndex+1; i++) {
				...
				question.addAnswer(blankIndex, line.substring(startIndex+5, endIndex));
				startIndex = line.indexOf("\n    ", startIndex+1); //finds next starting point
			}
